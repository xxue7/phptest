...
class PHPMailer {
	...
	public function addAttachment(
		...
		try {
			...
			$filename = (string) static::mbPathinfo($path, PATHINFO_BASENAME);
			if ('' === $name) {
				$name = $filename;
			}
	...
	public function addStringAttachment(
		...
		try {
			...
			$this->attachment[] = [
				0 => $string,
				1 => $filename,
				2 => static::mbPathinfo($filename, PATHINFO_BASENAME),
				3 => $encoding,
				4 => $type,
		...
		if (array_key_exists(2, $images)) {
			...
			foreach ($images[2] as $imgindex => $url) {
				...
				if ( // Only process relative URLs if a basedir is provided (i.e. no absolute local paths)
					...
					$filename = static::mbPathinfo($url, PATHINFO_BASENAME);
	...
	public function dkimAdd($headers_line, $subject, $body) {
		...
		foreach ($foundExtraHeaders as $key => $value) {
			...
			if ($this->DKIM_copyHeaderFields) {
				$extraCopyHeaderFields .= ' |' . str_replace('|', '=7C', $this->dkimQp($value)) . ";\r\n";
			}
		}
		if ($this->DKIM_copyHeaderFields) {
